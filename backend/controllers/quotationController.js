...
exports.convertToOrder = async (req, res) => {
  try {
    ...
    // Generate unique order number (will be auto-generated by pre-save hook)
    ...
    const order = await Order.create({
      ...
      // shippingAddress is stored in notes for quotations
    ...
    // Reserve inventory for each product (if product has reservations array)
    for (let item of quotation.items) {
      try {
        ...
        console.log('Could not update product reservations:', err.message);
        // Continue even if reservation fails
      }
    }
